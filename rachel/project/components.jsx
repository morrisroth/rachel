// Shared UI atoms used by both Field and HQ screens.

const { useState, useEffect, useRef, useMemo } = React;

// ─────────────────────────────────────────────────────────────────────────────
// Icons — simple inline SVGs, sized via currentColor + width prop.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Brand crest — small letter-mark "ר" inside a square. Used in top bars.
// ─────────────────────────────────────────────────────────────────────────────

function Crest({ subtitle }) {
  return (
    <div className="crest">
...
// ─────────────────────────────────────────────────────────────────────────────
// Status indicator block (used on field dashboard top)
// ─────────────────────────────────────────────────────────────────────────────

function StatusBlock({ status, lastReportMs }) {
  // status: 'ok' | 'missing' | 'soon'
  const map = {
...
// ─────────────────────────────────────────────────────────────────────────────
// Combobox — product picker with free-text fallback (PRD §5.2 / §11.2)
// ─────────────────────────────────────────────────────────────────────────────

function ProductCombobox({ value, onChange, onFreeText, products, categories, placeholder = 'בחר מוצר או הקלד שם מוצר חדש…' }) {
  // value: { kind: 'catalog', product_id } | { kind: 'free', name, category_id, unit }
    ...
    <div ref={ref} style={{position:'relative'}}>
      <button
        ...
        style={{
          ...
          cursor:'pointer', textAlign:'right', minHeight:42,
          color: displayLabel ? 'var(--ink)' : 'var(--ink-3)',
        }}
      >
      ...
      {open && (
        <div className="card anim-in" style={{
          ...
          maxHeight:320, overflow:'auto', zIndex:30, padding:6,
          boxShadow:'var(--shadow-2)',
        }}>
          <div style={{display:'flex', alignItems:'center', gap:8, padding:'6px 10px', borderBottom:'1px solid var(--line)'}}>
            <Icon name="search" size={14}/>
            <input
              className="input" style={{border:'0', padding:'4px 0', boxShadow:'none', fontSize:14}}
          ...
          {matches.length > 0 ? (
                ...
                return (
                  <button key={p.id} type="button"
                    ...
                    style={{
                      ...
                      padding:'9px 10px', border:'0', background:'transparent', cursor:'pointer', borderRadius:6,
            ...
            <div style={{padding:'18px 14px', color:'var(--ink-3)', fontSize:13}}>
              לא נמצא מוצר תואם בקטלוג.
            </div>
          )}
            ...
            <>
              ...
              <button type="button"
                ...
                style={{
                  ...
                  padding:'10px 10px', border:'0', background:'transparent', cursor:'pointer', borderRadius:6,
                  color:'var(--accent)', font:'500 14px var(--font-ui)',
                }}
...
// ─────────────────────────────────────────────────────────────────────────────
// Top bar — used on HQ desktop view
// ─────────────────────────────────────────────────────────────────────────────

function TopBar({ user, onLogout, right }) {
  return (
    <div className="topbar">
      <Crest subtitle="מטה רח״ל · חמ״ל מרכזי" />
      <div style={{display:'flex', alignItems:'center', gap:14}}>
        {right}
        <div style={{display:'flex', alignItems:'center', gap:10, paddingInlineStart:14, borderInlineStart:'1px solid var(--line)', height:32}}>
