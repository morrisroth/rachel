// Field-user screens — mobile-first, contained in the phone frame.
// Three tabs: דיווח (Dashboard + form), היסטוריה (History), פרופיל (small).
...
function emptyLine() {
  return {
    ...
    product: null,                  // { kind:'catalog'|'free', product_id|name, category_id?, unit? }
    category_id: null,              // mandatory; mirrors product.category_id when catalog, picked when free
    current_stock: '',
    incoming_stock: '',
...
function lineFromLinkedProduct(product_id) {
  ...
  return {
    ...emptyLine(),
    product: { kind: 'catalog', product_id },
...
function FieldShell({ user, org, history, onSubmit, onLogout, mode, viewport = 'phone' }) {
  const [tab, setTab] = useStateF('report');
  const [toast, setToast] = useStateF(null); // { text, kind:'ok'|'err' }
  ...
  const orgName = org?.name || ORGANIZATIONS.find(o => o.id === user.organization_id)?.name;
    ...
    return (
      <div style={{minHeight:'100vh', display:'flex', flexDirection:'column', background:'var(--bg)', position:'relative'}}>
        ...
        <header style={{
          ...
          borderBottom:'1px solid var(--line)', gap:24,
        }}>
          ...
          <nav style={{display:'flex', gap:2}}>
            {navItems.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                ...
                <Icon name={t.icon} size={15} stroke={tab === t.id ? 2 : 1.7}/>
                {t.label}
                {t.badge && tab !== t.id && (
                  <span style={{
        ...
        {toast && (
          <div className="anim-in" style={{
            ...
            boxShadow:'0 12px 32px oklch(0% 0 0 / .18)',
            zIndex:50,
          }}>
            <Icon name={toast.kind === 'ok' ? 'check' : 'alert'} size={16} stroke={2.4}/>
            {toast.text}
          </div>
        )}
      </div>
      ...
      <div className="phone">
        ...
        <div className="phone-body" style={{display:'flex', flexDirection:'column', position:'relative'}}>
          <div className="sec-h" style={{position:'sticky', top:0, zIndex:5}}>
            <div>
              <h2>{orgName}</h2>
              <div className="sub">{user.title}</div>
            </div>
            <div style={{display:'flex', gap:6, alignItems:'center'}}>
              ...
              <button className="btn btn--ghost btn--sm" onClick={onLogout} title="יציאה"><Icon name="logout" size={14}/></button>
            </div>
          </div>
          ...
          <div style={{
            ...
            {navItems.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                ...
                <Icon name={t.icon} size={20} stroke={tab === t.id ? 2 : 1.6}/>
                {t.label}
                {t.badge && (
                  <span style={{
                    position:'absolute', top:8, insetInlineStart:'calc(50% + 8px)',
                    width:7, height:7, borderRadius:'50%', background:'var(--bad)',
...
function ReportTab({ user, org, history, dailyOk, mode, onSubmit, viewport = 'phone' }) {
  // Initialize with one line per admin-linked product. If org has no linked
  // catalogue (e.g. seed/imported HQ user accidentally landing here), fall
  // back to a single empty line so the form still works.
  const [lines, setLines] = useStateF(() => {
    const linked = org?.linked_products || [];
    if (linked.length === 0) return [emptyLine()];
    return linked.map(pid => lineFromLinkedProduct(pid));
  });
  const [activeKey, setActiveKey] = useStateF(null);
  ...
  function submit() {
    ...
    onSubmit({ user_id: user.id, organization_id: user.organization_id, lines, reported_at: Date.now() });
  }

  return (
    <div style={{
      padding: isPc ? '28px 32px 80px' : '16px 16px 80px',
      display:'flex', flexDirection:'column', gap: isPc ? 18 : 14,
      maxWidth: isPc ? 1080 : '100%',
      ...
      <div style={{display:'flex', gap:8, flexWrap:'wrap'}}>
        ...
        {(org?.linked_products?.length > 0) && (
          <button className="btn btn--ghost" onClick={resetToLinked} title="חזרה לקטלוג שהוקצה לארגון על ידי המטה" style={{flex:'0 0 auto'}}>
      ...
      <div style={{
        ...
        <div style={{display:'flex', gap:6, flexWrap:'wrap'}}>
          ...
          <span className="chip" style={{fontSize:11}}>
            <Icon name="shield" size={11}/> מידור הרמטי
          </span>
        </div>
      </div>
      ...
      <div style={{
        ...
        bottom: isPc ? 16 : 'auto',
        display:'flex', flexDirection: isPc ? 'row' : 'column', gap:10, alignItems: isPc ? 'center' : 'stretch',
        background: isPc ? 'var(--bg)' : 'transparent',
        padding: isPc ? '12px 0' : 0,
        zIndex: 4,
      }}>
        <button className="btn btn--accent btn--lg" onClick={submit} style={{justifyContent:'center', flex: isPc ? '0 0 auto' : 1, minWidth: isPc ? 220 : 0}}>
          שלח דיווח מלאי
        </button>
  ...
  return (
    <div className={`card ${active ? 'row-hl' : ''}`} style={{padding:14, display:'flex', flexDirection:'column', gap:12}} onFocus={onFocus} onClick={onFocus}>
      <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:8}}>
        ...
        {onRemove && (
          <button className="btn btn--ghost btn--sm" onClick={onRemove} style={{padding:'6px 8px'}} title="מחק"><Icon name="trash" size={13}/></button>
        )}
      </div>
      ...
      <div>
        ...
        <ProductCombobox
          ...
          onChange={(v) => onPickCatalog(v.product_id)}
          onFreeText={(name) => onPickFree(name)}
        />
      ...
      {isFree && (
        <div>
          ...
          <div style={{font:'400 11px var(--font-ui)', color:'var(--ink-3)', marginTop:6}}>
            חובה — קובע למחיצה איזה משרד בקובץ הייצוא של המטה.
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{position:'relative'}}>
            <input
              type="number" min="0" step="0.01"
              className="input num-input"
              value={line.incoming_stock}
              onChange={e => onChange({ incoming_stock: e.target.value })}
              placeholder="0.00"
              style={{paddingInlineEnd: unit || isFree ? 56 : 12}}
            />
            <UnitBadge unit={unit} isFree={isFree} onChangeUnit={(u) => onChange({ unit: u })}/>
      ...
      <div>
        ...
        <input type="date" className="input"
          ...
          onChange={e => onChange({ expected_arrival_date: e.target.value })}/>
      </div>

      <div>
        <label className="label">הערות</label>
        <textarea className="input" rows={2} maxLength={200}
          placeholder="הערות נקודתיות, חריגות, או הקשר מבצעי…"
          value={line.notes}
...
function UnitBadge({ unit, isFree, onChangeUnit }) {
  ...
  if (isFree && !unit) {
    return (
      <select
        ...
        value={unit}
        style={{
          position:'absolute', insetInlineEnd:6, top:4, height:34,
          padding:'0 6px', border:'1px solid var(--line-2)', borderRadius:5,
          background:'var(--bg-2)', font:'500 12px var(--font-mono)', color:'var(--ink-2)',
        }}>
  ...
  return (
    <span className="mono" style={{
      ...
      padding:'3px 8px', borderRadius:4, background:'var(--bg-2)',
      border:'1px solid var(--line)', font:'600 12px var(--font-mono)', color:'var(--ink-2)',
    }}>{unit}</span>
  );
}
...
function HistoryTab({ user, history, isPc = false }) {
  return (
    <div style={{
      ...
      maxWidth: isPc ? 1080 : '100%', margin: isPc ? '0 auto' : 0, width:'100%',
    }}>
      <div style={{font:'600 13px var(--font-ui)', color:'var(--ink-2)', letterSpacing:'.04em', textTransform:'uppercase'}}>
        היסטוריית דיווחים · קריאה בלבד
      </div>
      <div style={{font:'400 12px var(--font-ui)', color:'var(--ink-3)', marginTop:-8}}>
        מציג את כל דיווחי הארגון שלך, מסודרים מהחדש לישן. מסונן בשרת לפי organization_id מהפעלת ההפעלה — לא ניתן לראות נתוני ארגון אחר.
      </div>
      {history.map(rep => (
          ...
          <div>
            {rep.lines.map((l, i) => {
              ...
              return (
                <div key={i} style={{
                  padding:'12px 14px',
                  borderBottom: i < rep.lines.length - 1 ? '1px solid var(--line)' : 0,
  ...
  return (
    <div style={{padding:'24px 18px 80px', display:'flex', flexDirection:'column', gap:18, maxWidth:680, margin:'0 auto', width:'100%'}}>
      <div className="card" style={{padding:18, display:'flex', flexDirection:'column', gap:12}}>
        ...
        <div className="hr"/>
        <Row label="ארגון"        value={org?.name || '—'}/>
        <Row label="תעודת זהות"   value={<span className="mono">{user.id_number}</span>}/>
        <Row label="טלפון"        value={<span className="mono">{user.phone}</span>}/>
        <Row label="הרשאה"       value={<span className="chip chip--accent" style={{fontSize:11}}>{user.role}</span>}/>
      </div>
        ...
        <div className="card" style={{padding:0, overflow:'hidden'}}>
          <div style={{padding:'12px 16px', borderBottom:'1px solid var(--line)', background:'var(--bg-2)'}}>
            ...
            <div style={{font:'400 12px var(--font-ui)', color:'var(--ink-3)', marginTop:2}}>
              {linkedProducts.length} מוצרים שמטה רח״ל הקצה לארגון שלך. אלו השורות שייפתחו אוטומטית במסך הדיווח.
            </div>
          </div>
          {linkedProducts.map((p, i) => (
            <div key={p.id} style={{
              padding:'10px 16px',
              borderBottom: i < linkedProducts.length - 1 ? '1px solid var(--line)' : 0,
              display:'flex', alignItems:'center', justifyContent:'space-between',
            }}>
              <div>
      ...
      <div style={{font:'400 11px var(--font-ui)', color:'var(--ink-3)', textAlign:'center'}}>
        רחל · גרסת MVP · רשת אזרחית
      </div>
    </div>
  );
}
...
Object.assign(window, { FieldShell });
